...
def test_resolves_preconverted_eole_hf_repo(tmp_path):
    ...
    with (
        ...
        patch("eole.models.hf_resolver._local_dir_is_loadable_eole_model", side_effect=lambda _: next(cache_checks)),
    ):
...
def test_list_repo_files_failure_without_cache_returns_none(tmp_path):
    cache_dir = tmp_path / "owner--model"
    ...
    with (
        ...
        assert resolve_preconverted_eole_hf_repo("owner/model") is None


def test_remote_eole_config_download_failure_raises_clear_error(tmp_path):
    cache_dir = tmp_path / "owner--model"
...
def test_remote_eole_download_failure_uses_valid_cache(tmp_path):
    cache_dir = tmp_path / "owner--model"
    cache_dir.mkdir()
    (cache_dir / "config.json").write_text(json.dumps({"model": {}, "training": {}}), encoding="utf-8")
    (cache_dir / "vocab.json").write_text("{}", encoding="utf-8")
...
def test_remote_eole_download_failure_without_cache_raises_clear_error(tmp_path):
    cache_dir = tmp_path / "owner--model"
...
def test_non_eole_hf_repo_is_not_resolved(tmp_path):
    with (
        patch(
            "eole.models.hf_resolver.list_repo_files",
...
def test_build_predictor_loads_preconverted_hf_repo_as_local_eole_model():
    ...
    assert config.model_path == ["/tmp/eole-model"]
...
def test_build_predictor_falls_back_to_generic_hf_loader():
    class FakePredictor:
        def __init__(self, *args, **kwargs):
            pass
...
def test_comet_scorer_resolves_preconverted_hf_repo():
    with (
        patch.dict("os.environ", {}, clear=True),
        patch("eole.scorers.eole_comet.resolve_preconverted_eole_hf_repo", return_value="/tmp/comet") as resolve,
    ):
        assert resolve_comet_model_dir("owner/comet") == "/tmp/comet"

    resolve.assert_called_once_with("owner/comet")


def test_metricx_scorer_resolves_preconverted_hf_repo():
    with (
        patch.dict("os.environ", {}, clear=True),
        patch("eole.scorers.eole_metricx.resolve_preconverted_eole_hf_repo", return_value="/tmp/metricx") as resolve,
    ):
        assert resolve_metricx_model_dir("owner/metricx") == "/tmp/metricx"

    resolve.assert_called_once_with("owner/metricx")


def test_missing_model_config_errors_are_clear():
