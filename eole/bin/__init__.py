...
def register_bin(name):
    ...
    def register_bin_cls(cls):
        ...
        if bin_type in AVAILABLE_BINS:
            if name in AVAILABLE_BINS[bin_type] and cls != AVAILABLE_BINS[bin_type][name]:
                raise ValueError(
...
for dir in os.listdir(bin_dir):
    ...
    if os.path.isdir(abs_dir):
        for file in os.listdir(abs_dir):
            ...
            if not file.startswith("_") and not file.startswith(".") and (file.endswith(".py") or os.path.isdir(_path)):
