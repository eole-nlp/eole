...
class SwitchOutConfig(TransformConfig):
    switchout_temperature: float | None = Field(
        default=1.0,
        description="Sampling temperature for SwitchOut. :math:`\\tau^{-1}`"
...
class SwitchOutTransform(HammingDistanceSamplingTransform):
    ...
    def _parse_config(self):
        self.temperature = self.config.switchout_temperature
...
class TokenDropTransform(HammingDistanceSamplingTransform):
    ...
    config_model = TokenDropConfig

    def __init__(self, config):
        super().__init__(config)

    def _parse_config(self):
        self.temperature = self.config.tokendrop_temperature
...
class TokenMaskTransform(HammingDistanceSamplingTransform):
    ...
    def _parse_config(self):
        self.temperature = self.config.tokenmask_temperature
