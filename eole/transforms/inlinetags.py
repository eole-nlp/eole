...
class InlineTagsConfig(TransformConfig):
    ...
    src_delimiter: str | None = Field(
        default="｟fuzzy｠",
        description="Any special token used for augmented src sentences. "
        "The default is the fuzzy token used in the FuzzyMatch transform.",
    )
...
class InlineTagsTransform(Transform):
    ...
    config_model = InlineTagsConfig

    def __init__(self, config):
        super().__init__(config)

    def _parse_config(self):
        self.tags_dictionary_path = self.config.tags_dictionary_path
        self.tags_corpus_ratio = self.config.tags_corpus_ratio
        self.max_tags = self.config.max_tags
        self.src_delimiter = self.config.src_delimiter
    ...
    def get_specials(cls, config):
        ...
        tags = list()
        for i in range(1, config.max_tags * 2):
            tags.extend(
                [
    ...
    def warm_up(self, vocabs=None):
        ...
        self.tagger = InlineTagger(
            ...
            self.max_tags,
            self.config.paired_stag,
            self.config.paired_etag,
            self.config.isolated_tag,
            self.src_delimiter,
