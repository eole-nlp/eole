...
class LossCompute(nn.Module):
    ...
    def from_config(cls, config, model, vocabs, train=True):
        ...
        if config.training.lm_prior_model:
            if config.training.lm_prior_model[-3:] == ".pt":
                _, lm_prior_model, lm_model_config = DecoderModel.for_inference(
    ...
    def _compute_attention_entropy(self, attns):
        ...
        """
        attention_available = (
            attns is not None and isinstance(attns, dict) and any(attn is not None for attn in attns.values())
        )
