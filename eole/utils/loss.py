...
class LossCompute(nn.Module):
    ...
    def from_config(cls, config, model, vocabs, train=True):
        ...
        compute.log_attention_entropy = getattr(config.training, 'log_attention_entropy', True)
    ...
    def forward(self, batch, output, attns, estim=None):
        ...
        if (hasattr(self, 'log_attention_entropy') and self.log_attention_entropy and 
            attns and any(attn is not None for attn in attns.values())):
            try:
    ...
    def _stats(self, bsz, loss, auxloss, scores, target, cids, cids_idx, attention_entropy=0.0):
        ...
        return eole.utils.Statistics(
            ...
            data_stats=data,
            attention_entropy=attention_entropy * bsz,  # Scale by batch size for proper averaging
